...
@dataclass
class TrainingArguments(args.TrainingArguments):
...
class DPODataset(Dataset):
    ...
    def __init__(self, data_mixture: str, tokenizer: transformers.PreTrainedTokenizer, data_args: args.DataArguments):
...
def train():
    ...
    parser = HfArgumentParser((args.ModelArguments, args.DataArguments, TrainingArguments))
...
`"""
    ...
    if training_args.lora_enable:
        ...
        if training_args.tune_decoder_layers > 0:
            assert training_args.tune_language_model, "You need to set tune_language_model to True if you want to tune decoder layers."
            assert training_args.tune_decoder_layers <= len(
                model.get_llm().model.layers
            ), f"tune_decoder_layers {training_args.tune_decoder_layers} exceeds the total number of layers {len(model.get_llm().model.layers)}"
            
            # set the whole llm to not require grad first
            model.get_llm().requires_grad_(False)
            for p in model.get_llm().model.layers[-training_args.tune_decoder_layers:].parameters():
    ...
    if training_args.importance_dir and training_args.train_low_importance_ratio > 0.0:
        ...
        if not using_lora:
            ...
            raise NotImplementedError("Selective FT with LoRA is not implemented yet.")
