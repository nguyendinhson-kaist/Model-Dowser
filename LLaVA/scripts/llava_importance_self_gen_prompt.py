...
def collect_linear_modules_llava(
...
def sample_self_text_ids_batch(
    ...
    # add the image token id into the beginning of the synthetic sequence
    if not image_off:
...
def main():
    ...
    tokenizer, model, image_processor, context_len = load_pretrained_model(
        ...
        model_base=args.model_base if args.model_base is not None else None,
    ...
    modules = collect_linear_modules_llava(
        model,
    ...
    H = (getattr(image_processor, "crop_size", {}) or {}).get("height", 336)
    W_img = (getattr(image_processor, "crop_size", {}) or {}).get("width", 336)
    C = 3
